...
import { WorkMethodSettings } from '@/components/settings/WorkMethodSettings';
...
const Settings = () => {
  ...
  const [settingsCodeInput, setSettingsCodeInput] = useState('');
  const [advancedTabsUnlocked, setAdvancedTabsUnlocked] = useState(() => {
    if (typeof window === 'undefined') return false;
    return window.localStorage.getItem('settings-advanced-tabs-unlocked') === '1';
  });
  ...
  useEffect(() => {
    if (!advancedTabsUnlocked && (activeTab === 'pricing' || activeTab === 'work_methods')) {
      setActiveTab('operations');
    }
  }, [advancedTabsUnlocked, activeTab]);
  ...
  const toggleAdvancedTabsAccess = () => {
    if (advancedTabsUnlocked) {
      setAdvancedTabsUnlocked(false);
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('settings-advanced-tabs-unlocked', '0');
      }
      toast({
        title: tr('Modules masqués', 'تم إخفاء الوحدات'),
        description: tr('Paramétrage et Méthode de travail sont maintenant cachés.', 'تم الآن إخفاء التهيئة وطريقة العمل.')
      });
      return;
    }

    if (settingsCodeInput.trim() !== SETTINGS_CODE) {
      toast({
        title: tr('Code invalide', 'رمز غير صحيح'),
        description: tr('Veuillez saisir le code correct pour afficher les modules.', 'يرجى إدخال الرمز الصحيح لإظهار الوحدات.'),
        variant: 'destructive'
      });
      return;
    }

    setAdvancedTabsUnlocked(true);
    setSettingsCodeInput('');
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('settings-advanced-tabs-unlocked', '1');
    }
    toast({
      title: tr('Modules affichés', 'تم إظهار الوحدات'),
      description: tr('Paramétrage et Méthode de travail sont maintenant disponibles.', 'أصبحت الآن التهيئة وطريقة العمل متاحتين.')
    });
  };
          ...
          <Card className="border-slate-800 bg-slate-900/60">
            <CardContent className="pt-6">
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                {!advancedTabsUnlocked && (
                  <Input
                    value={settingsCodeInput}
                    onChange={(e) => setSettingsCodeInput(e.target.value)}
                    placeholder="Code d'accès (654321)"
                    className="md:max-w-xs border-slate-700 bg-slate-950 text-slate-100"
                  />
                )}
                <Button
                  type="button"
                  onClick={toggleAdvancedTabsAccess}
                  className={advancedTabsUnlocked ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700'}
                >
                  {advancedTabsUnlocked ? 'Cacher modules protégés' : 'Afficher modules par code'}
                </Button>
                <span className="text-xs text-slate-400">
                  {advancedTabsUnlocked
                    ? 'Modules protégés visibles: Paramétrage, Méthode de travail'
                    : 'Modules protégés masqués: Paramétrage, Méthode de travail'}
                </span>
              </div>
            </CardContent>
          </Card>

          <TabsList className={`grid w-full ${advancedTabsUnlocked ? 'grid-cols-5' : 'grid-cols-3'} border border-slate-700 bg-slate-900/90`}>
            ...
            {advancedTabsUnlocked && (
              <TabsTrigger value="work_methods" className="text-slate-200 hover:text-white data-[state=active]:bg-slate-700 data-[state=active]:text-white">
                {tr('Méthode de travail', 'طريقة العمل')}
              </TabsTrigger>
            )}
          </TabsList>

          {advancedTabsUnlocked && (
            <TabsContent value="work_methods" className="space-y-6 m-0">
              <WorkMethodSettings />
            </TabsContent>
          )}
