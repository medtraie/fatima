...
const SupplyReturn = () => {
  ...
  const dashboardStats = useMemo(() => {
    ...
    const paidReturns = filteredReturnOrders.filter((order: any) => isReturnPaid(order)).length;
  ...
  const commandTimeline = useMemo(() => {
    ...
    const returnFlow = filteredReturnOrders.map((order: any) => ({
      ...
      amountLabel: isReturnPaid(order) ? tr('Réglé', 'مسدد') : tr('En attente', 'قيد الانتظار')
    }));
